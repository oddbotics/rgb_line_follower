...
grid_follower::grid_follower(){
  ...
  private_node_handle_.param<double>("angular", angular, 1.0);
  private_node_handle_.param<double>("tilt_angle", tilt_angle, -0.36);
  private_node_handle_.param<bool>("isCCW", isCCW, true);
    
  //initialize the publishers and subscribers
  cmd_pub = nh.advertise<geometry_msgs::Twist>("cmd_vel", 1000);
  camera_pub = nh.advertise<oddbot_msgs::HerkulexCommand>("perce_herkulex_command" , 1000, true);
  image_sub = nh.subscribe(image_topic, 1000, &grid_follower::update_image, this); 
  
  //tilt the camera up
  oddbot_msgs::HerkulexCommand camera_command;
  camera_command.name.push_back(2);
  camera_command.position.push_back(tilt_angle);

  camera_pub.publish(camera_command);
}
